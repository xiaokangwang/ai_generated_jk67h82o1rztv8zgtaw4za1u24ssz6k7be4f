...
pub(crate) struct ClientHelloPayload {
    ...
    pub(crate) craft_extensions: Option<Vec<crate::craft::CraftClientExtension>>,
}
...
impl ClientHelloPayload {
    ...
    pub(crate) fn payload_encode(&self, bytes: &mut Vec<u8>, purpose: Encoding) {
        ...
        let to_compress = match purpose {
            ...
            _ => {
                if let Some(craft_extensions) = &self.craft_extensions {
                    craft_extensions.encode(bytes);
                } else {
    ...
    pub(crate) fn collect_used(&self) -> Vec<ExtensionType> {
        match &self.craft_extensions {
            Some(extensions) => extensions
                .iter()
                .map(|extension| extension.extension_type())
                .collect(),
            None => self.extensions.collect_used(),
        }
    }
}
